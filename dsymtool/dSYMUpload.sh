...
# # 注意: 请配置下面的信息
################################################################################
BUGLY_APP_ID="e35021ccc5"
BUGLY_APP_KEY="f781f3c3-50e4-45d6-bd2a-f83876b55d3e"
BUNDLE_IDENTIFIER="com.dzpqzb.chatdaily"
################################################################################
# # 自定义配置
################################################################################
...
function uploadDSYM {
    ...
    echo "curl --header "Content-Type:application/zip" --data-binary "@${DSYM_ZIP_FPATH}" "${DSYM_UPLOAD_URL}" --verbose -s"
            
    echo "--------------------------------"
    STATUS=$(/usr/bin/curl --header "Content-Type:application/zip" --data-binary "@${DSYM_ZIP_FPATH}" "${DSYM_UPLOAD_URL}" --verbose -s)
    echo "--------------------------------"
